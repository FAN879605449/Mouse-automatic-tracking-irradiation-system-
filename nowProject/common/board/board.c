...
{
#if defined(DEBUG_UART)   // #define DEBUG_UART LPC_USART1
	static char prevChar = 0;
	if (ch == '\n' && prevChar != '\r')  // ch换行 * prevChar非回车
	{
		prevChar = '\r';
		Chip_UART_SendBlocking(DEBUG_UART, &prevChar, 1);  /* Transmit a byte array through the UART peripheral (blocking) */
		Chip_UART_SendBlocking(DEBUG_UART, &ch, 1);
	} else {
		Chip_UART_SendBlocking(DEBUG_UART, &ch, 1);
		prevChar = ch;
...
{
#if defined(DEBUG_UART)
	Board_UART_Init();    // 这里面完成了对 DEBUG_UART的引脚分配
