 ...
 */
typedef struct {
	__IO uint32_t  CFG;				/*!< Configuration register 基础UART配置 */
	...
	__IO uint32_t  BRG;				/*!< Baud Rate Generator register 波特率发生寄存器 */
	__IO uint32_t  INTSTAT;			/*!< Interrupt status register 反映当前已使能的中断 */
	__IO uint32_t  OSR;             /*!< Oversampling Selection regiser 过采样选择寄存器，用于异步通信*/
	__IO uint32_t  ADDR;            /*!< Address register for automatic address matching 用于自动匹配地址 */
} LPC_USART_T;
...
{
	pUART->CFG = UART_CFG_ENABLE | (pUART->CFG & ~UART_CFG_RESERVED);      //configuration配置
}
