...
int Chip_UART_Send(LPC_USART_T *pUART, const void *data, int numBytes)
{
	...
	/* Send until the transmit FIFO is full or out of bytes 等到FIFO的发送端溢满时就发送 */
	while ((sent < numBytes) &&
		   ((Chip_UART_GetStatus(pUART) & UART_STAT_TXRDY) != 0)) 
	{
		Chip_UART_SendByte(pUART, *p8);  // pUART->TXDATA = (uint32_t) data; 往发送端赋值
		p8++;
		sent++;
	}
...
/* Transmit a byte array through the UART peripheral (blocking在没有返回值时，一直等待) */
int Chip_UART_SendBlocking(LPC_USART_T *pUART, const void *data, int numBytes)
{
	...
	while (numBytes > 0)
	{
		pass = Chip_UART_Send(pUART, p8, numBytes);  // 非阻塞传送字节，return(传了多少次)
		numBytes -= pass;  // 还剩多个
		sent += pass;   //阻塞传了多少个
		p8 += pass;  //指针的位置后移
	}

	return sent;       // 阻塞传了多少字节
}
